...
datasetvar = subset(dataset36mon, select = c("int.rate", "grade", "sub.grade", "home.ownership", "annual.inc", "delinq.2yrs", "dti", "inq.last.6mths", "installment", "loan.status", 
    "loan.amnt", "mths.since.last.delinq", "open.acc", "pub.rec", "purpose", "revol.util", "earliest.cr.line", "issue.d"))
set.seed(123)
...
# creating new variable, since two borrower indebtedness variables are not directly given creating loan amount to annual income
...
summary(useddata)
# months since last delinquency #has aroub half observations sind without value replace NA with negative value, than convert the numeric variable into categories
...
plotIntRate = ggplot(useddata, aes(int.rate, fill = loan.status)) + geom_density(alpha = 0.5)
plotAnnInc = ggplot(useddata, aes(annual.inc, fill = loan.status)) + geom_density(alpha = 0.5)
plotDelinq = ggplot(useddata, aes(delinq.2yrs)) + geom_bar(aes(fill = loan.status), position = "fill")
plotDti = ggplot(useddata, aes(dti, fill = loan.status)) + geom_density(alpha = 0.5)
...
plotCred = ggplot(useddata, aes(length.credit, fill = loan.status)) + geom_density(alpha = 0.5)

plotNum = grid.arrange(plotIntRate, plotAnnInc, plotDelinq, plotDti, plotInq, plotAmt, plotOpenacc, plotPudrec, plotRevol, plotLoaninc, plotInsinc, plotCred, nrow = 6, top = "Plots of numeric Variables")
